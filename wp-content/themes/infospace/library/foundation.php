...
if ( ! function_exists( 'startertheme_get_the_comments_pagination' ) ) :
	function startertheme_get_the_comments_pagination( $args = array() ) {

		global $namespace;
		$navigation = '';
		$args = wp_parse_args( $args, array(
			'prev_text'				=> __( '&laquo;', $namespace ),
			'next_text'				=> __( '&raquo;', $namespace ),
			'size'					=> 'default',
			'show_disabled'			=> true,
		) );
		$args['type'] = 'array';
...
// Custom Comments Pagination.
if ( ! function_exists( 'startertheme_the_comments_pagination' ) ) :
	function startertheme_the_comments_pagination( $args = array() ) {
		echo startertheme_get_the_comments_pagination( $args );
	}
endif;
...
if ( ! function_exists( 'startertheme_menu_fallback' ) ) :
	function startertheme_menu_fallback() {
		global $namespace;

		echo '<div class="alert-box secondary">';
		/* translators: %1$s: link to menus, %2$s: link to customize. */
		printf(
			__( 'Please assign a menu to the primary menu location under %1$s or %2$s the design.', $namespace ),
			/* translators: %s: menu url */
			sprintf(
				__( '<a href="%s">Menus</a>', $namespace ),
				get_admin_url( get_current_blog_id(), 'nav-menus.php' )
			),
			/* translators: %s: customize url */
			sprintf(
				__( '<a href="%s">Customize</a>', $namespace ),
...
if ( ! function_exists( 'startertheme_active_nav_class' ) ) :
	function startertheme_active_nav_class( $classes, $item ) {
		if ( $item->current == 1 || $item->current_item_ancestor == true ) {
			$classes[] = 'is-active';
		}
		return $classes;
	}
	add_filter( 'nav_menu_css_class', 'startertheme_active_nav_class', 10, 2 );
endif;
 ...
 */
if ( ! function_exists( 'startertheme_active_list_pages_class' ) ) :
	function startertheme_active_list_pages_class( $input ) {
	...
	add_filter( 'wp_list_pages', 'startertheme_active_list_pages_class', 10, 2 );
endif;
 ...
 */
if ( ! function_exists( 'startertheme_gallery' ) ) :
	function startertheme_gallery($attr) {
	...
	add_shortcode('gallery', 'startertheme_gallery');
endif;
